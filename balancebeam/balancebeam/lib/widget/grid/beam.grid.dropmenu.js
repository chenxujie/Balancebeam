...
$.ui.grid.dropmenu.prototype = {
	...
	_create : function(){
		...
		headersNode.bind("mousedown.grid-dropmenu",function(e){
			...
			if(columnMarkup.view == grid.normalView){ //不是非锁定视图
				dropMenu.menu("setDisabled","lock",false);
	...
	createPopMenu : function(){
		...
		dropMenu.menu({
			...
			dataProvider : [
			{
				...
				disabledIcon : incoPath + "grid-menu-asc-disabled.gif"
			},{
				id : "desc",
				title : "降序",
				icon : incoPath + "grid-menu-desc.gif",
				disabledIcon : incoPath + "grid-menu-desc-disabled.gif"
			},{
				seperator : true
			},
			{
				id : "lock",
				title : "锁定",
				icon : incoPath + "grid-menu-lock.gif",
				disabledIcon : incoPath + "grid-menu-lock-disabled.gif"
			},{
				id : "unlock",
				title : "解锁",
				icon : incoPath + "grid-menu-unlock.gif",
				disabledIcon : incoPath + "grid-menu-unlock-disabled.gif"
			}],
	...
	lock : function(index){
		...
		if(1==grid.normalView.columns.length){
			alert("至少存在一个可见非锁定列。");
			return;
		}
