...
$.ui.grid.scroller.prototype = {
	...
	funnelEvents : function(){
		...
		this.grid.contentsNode.bind(
				document.attachEvent ? "mousewheel.grid-scroller":"DOMMouseScroll.grid-scroller",function(e){
			$.stopEvent(e);
					var delta = 0,
				step =3;
			if (null!=e.wheelDelta) {
				delta = e.wheelDelta/120;
		    } else if (null!=e.detail) {
		    	delta = -e.detail/3;
		    }
			if(delta>0){ //向上滚动
	...
	scrollMoveY : function(e){
		...
		this.yscrollerBarNode.setCapture && this.yscrollerBarNode.setCapture();
		...
		function mouseupEvent(e){
			self.yscrollerBarNode.releaseCapture && self.yscrollerBarNode.releaseCapture();
	...
	scrollMoveX : function(e){
		...
		this.xscrollerBarNode.setCapture && this.xscrollerBarNode.setCapture();
		function mousemoveEvent(e){
			$.stopEvent(e);
		...
		function mouseupEvent(e){
			self.xscrollerBarNode.releaseCapture && self.xscrollerBarNode.releaseCapture();
			$(document).unbind("mousemove",mousemoveEvent);
			$(document).unbind("mouseup",mouseupEvent);
		}
		$(document).bind("mousemove",mousemoveEvent);
	...
	resize : function(xscrollerWidth,yscrollerHeight){
		...
		$(">.contents>.placeholder",grid.element).remove();
		if(viewport.yscroller && viewport.xscroller){
			...
			$(">.contents",grid.element).append($("<div class='placeholder'></div>"))
		}
	...
	destroy : function(){
		...
		this.grid.contentsNode.unbind(document.attachEvent ? "mousewheel.grid-scroller":"DOMMouseScroll.grid-scroller");
