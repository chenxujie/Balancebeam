 ...
 $.widget("ui.grid", $.ui.container,{
	...
	resize : function(wr){
		...
		if("auto"==options.height){
			...
			element.height(height + extendsHeight + (!options.hideHeader ? (headerNodeHeight+1) : 0));
		}
		else{
			//当显示表头时计算view的高度
		...
		if("auto"==options.width){
			...
			for (var i = 0, view; view = this.views[i]; i++) {
				...
				if(view.noscroll && this.views.length-2 == i){
					width++;
    			}
			}
			element.width(width);
	...
	setDataProvider : function(dataProvider){
		...
		this.runtime.dataProvider = dataProvider;
		if(this.normalView){
